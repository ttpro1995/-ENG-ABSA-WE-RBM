    ...
    for i in range(len(sentences[number_in_training_set:])):
        ...
        for j in range(len(words)):
            if words[j] in w2v_model.vocab: # Thử xem trong mô hình có từ đó không
                words_in_vocabs.append(words[j])
        if (words_in_vocabs != []):
            positive_point = w2v_model.n_similarity(words_in_vocabs, ['ngon','đẹp','sang','tốt','tuyệt','đậm','nhanh','thân','vui'])
            negative_point = w2v_model.n_similarity(words_in_vocabs, ['dở','tệ','lạt','phiền','chán','lâu','không'])
